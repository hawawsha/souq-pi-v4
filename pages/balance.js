...
export default function BalancePage() {
  ...
  useEffect(() => {
    // Load notification count using saved username
    const username = typeof localStorage !== 'undefined' && localStorage.getItem('pi_username');
    if (username) {
      fetch(`/api/client-notifications?username=${encodeURIComponent(username)}`)
        .then(r => r.json())
        .then(d => setNotifCount((d.records || []).length))
        .catch(() => {});
    }
  }, []);
  ...
  async function checkBalance() {
    ...
    setLoading(true); setBalance(null); setError('');
    try {
      ...
      else setError(data.error || 'لم يتم العثور على المحفظة');
    ...
    <>
      ...
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .card{background:rgba(106,13,173,0.1);border:1px solid #6a0dad;border-radius:20px;padding:24px;}
        ...
        .result-box{background:#1a0b2e;border:1px solid #331a5e;border-radius:16px;padding:20px;margin-top:20px;text-align:center;}
      ...
      <div className="container">
        <div className="card">
          <div style={{ textAlign:'center', marginBottom:20 }}>
            ...
            <div style={{ fontSize:'0.8em', color:'#b0b0b0' }}>استعلم عن رصيدك بإدخال عنوان المحفظة</div>
          </div>
          <input
            className="input"
            type="text"
          ...
          {balance !== null && (
            <div className="result-box">
              <div style={{ color:'#b0b0b0', fontSize:'0.82em', marginBottom:8 }}>الرصيد المتاح</div>
      ...
      <div className="bottom-nav">
        ...
        <div className="nav-wrap">
          <span style={{ fontSize:'1.3em' }}>💰</span>
          <span className="nav-label active">الرصيد</span>
        </div>
      </div>
    </>
  );
